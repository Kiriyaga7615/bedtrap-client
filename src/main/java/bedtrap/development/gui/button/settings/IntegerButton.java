...
public class IntegerButton extends SettingButton {
    ...
    IntegerButton(Module module, Setting setting, int X, int Y, int W, int H) {
    ...
    public static class Slider extends IntegerButton {
        private final Setting<Integer> intSetting;
        ...
        protected void updateSlider(final int mouseX) {
            ...
            if (dragging) {
                if (diff == 0.0) {
                    ...
                    intSetting.setValue((int) Double.parseDouble(String.format("%." + intSetting.getInc() + "f", diff / getW() * (max - min) + min).replace(",", ".")));
                }
            }
        }
    }
